...
export DATA_DIR=datasets/isaiah_hc_context_finetuned_paraphraser #paranmt_filtered
...
python3 -m torch.distributed.launch --nproc_per_node=1 $BASE_DIR/run_lm_finetuning.py \
    --output_dir=$BASE_DIR/saved_models/isaiah_hc_finetuned_paraphraser \
    --model_type=gpt2 \
    ...
    --job_id isaiah_hc_finetuning_paraphraser \
    --learning_rate 5e-5 \
