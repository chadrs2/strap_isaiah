...
export DATA_DIR=datasets/isaiah_context_eng
...
python3 -m torch.distributed.launch --nproc_per_node=1 $BASE_DIR/run_lm_finetuning.py \
    --output_dir=$BASE_DIR/saved_models/model_custom_isaiah_context \
    --model_type=gpt2 \
    --model_name_or_path=english_1990 \
    --do_train \
    --data_dir=$DATA_DIR \
    --save_steps 500 \
    --logging_steps 20 \
